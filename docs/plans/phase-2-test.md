...
| G2  | Agent refuses to fabricate      | Send: "Create a meeting with John at 3pm tomorrow"                                           | Agent should NOT claim it created anything (no `create_calendar_event` tool exists yet in Slice 1). Should explain it can currently only view calendar events.    |        |
